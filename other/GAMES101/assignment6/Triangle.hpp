...
public:
    ...
    Triangle(Vector3f _v0, Vector3f _v1, Vector3f _v2, Material* _m = nullptr)
        ...
        e1 = v1 - v0;
        e2 = v2 - v0;
        normal = normalize(crossProduct(e1, e2)); // 注意上面的 counter-clockwise order，normal 的向里向外是有讲究的，后面的交点计算也是涉及这一点
    }
...
inline Intersection Triangle::getIntersection(Ray ray) {
    ...
    if (dotProduct(ray.direction, normal) > 0) // 因为光线方向指的是从相机出发到三角形的方向，如果大于 0 说明光线与三角形背对，吗？
        return inter;
    double u, v, t_tmp = 0;
...
public:
    MeshTriangle(const std::string &filename) {
        ...
        for (int i = 0; i < mesh.Vertices.size(); i += 3) {
            ...
            auto new_mat =
                new Material(MaterialType::DIFFUSE_AND_GLOSSY, Vector3f(0.5, 0.5, 0.5), Vector3f(0, 0, 0)); // 没有 emission 自发光
            new_mat->Kd = 0.6;
            new_mat->Ks = 0.0;
        ...
        bvh = new BVHAccel(ptrs, 1, BVHAccel::SplitMethod::SAH); // 为什么用 SAH 反而慢啊啊啊
    }
    ...
    std::vector<Triangle> triangles; // 在 MeshTriangle 里面再套了一组 Triangle？好怪，为什么要这样设计
