...
public:
   ...
   // 将光源进行按面采样，随机从光源发射一条 ray 打到场景中的 sphere 上得到某个交点
   // 这里的意思是把 sphere 看作光源对其 sample，这是为了递归计算间接光照
    void Sample(Intersection &pos, float &pdf) {
        float theta = 2.0 * M_PI * get_random_float(), phi = M_PI * get_random_float();
        Vector3f dir(std::cos(phi), std::sin(phi)*std::cos(theta), std::sin(phi)*std::sin(theta)); // 经典球坐标系算法
        pos.coords = center + radius * dir;
        pos.normal = dir;
        pos.m = m;   // pos.emit = m->getEmission();
        pdf = 1.0f / area;
    }
    float getArea() {
        return area;
