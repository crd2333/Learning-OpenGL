...
public:
    ...
    // 将光源进行按面采样，随机从光源发射一条 ray 打到场景中的 triangle 上得到某个交点
    // 这里的意思是把 triangle 看作光源对其 sample，这是为了递归计算间接光照
    // 这里应该是想采样重心坐标，为了满足 alpha, beta, gamma in [0, 1) 的条件做了点变换
    // 但我有点不理解这样难道不会导致三个顶点不对称吗》
...
// MeshTriangle 是一大堆 triangles 的组合，它自己内部使用一个 BVH
// 想了一下感觉这种设计确实是有道理的（只是我依旧不知道为什么我用 SAH 没有明显加速甚至有点减速）
...
public:
    MeshTriangle(const std::string &filename, Material* mt = new Material()) {
        ...
        bvh = new BVHAccel(ptrs, 1, BVHAccel::SplitMethod::NAIVE); // 每个 MeshTriangle 有一个 BVH
    }
    ...
    void Sample(Intersection &pos, float &pdf) { // 本该是 meshtriangle 的采样任务又丢给 bvh，让 bvh 取采样小的 triangle
        bvh->Sample(pos, pdf);
