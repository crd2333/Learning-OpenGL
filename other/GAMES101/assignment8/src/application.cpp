...
void Application::init() {
    ...
    glPointSize(8);  // 设置点的大小为 8 像素
    glLineWidth(4); // 绘制线条的宽度为 4 像素

    glColor3f(1.0, 1.0, 1.0); // 设置颜色为白色

    // Create two ropes: start, end, num_nodes, node_mass, k, pinned_nodes
...
void Application::render() {
    ...
    // OpenGL 绘制
    for (int i = 0; i < 2; i++) { // 分别运行两个绳子模拟
        if (i == 0) {
            glColor3f(0.0, 0.0, 1.0); // Euler 方法用蓝色
            rope = ropeEuler;
        } else {
            glColor3f(0.0, 1.0, 0.0); // Verlet 方法用绿色
            rope = ropeVerlet;
        }
