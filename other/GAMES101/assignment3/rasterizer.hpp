...
private:
    ...
    // 函数对象，类似于 python 里面把函数当做变量传递
    std::function<Eigen::Vector3f(fragment_shader_payload)> fragment_shader; // 片元着色器，输入相关信息，输出颜色
    std::function<Eigen::Vector3f(vertex_shader_payload)> vertex_shader;     // 顶点着色器，输入顶点信息，输出位置（啥玩意儿

     // 对某一帧，width * height 的数据
    std::vector<Eigen::Vector3f> frame_buf;
    std::vector<float> depth_buf;
    int get_index(int x, int y);
    int width, height;

    // pos, ind, col, nor buffer 都是 map，它们共用指针，对象的内容直接按值存在 map 中
    int next_id = 0;
