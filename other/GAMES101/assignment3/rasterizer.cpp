...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    float xmax = std::max(v[0].x(), std::max(v[1].x(), v[2].x())) + 1;
    float ymin = std::min(v[0].y(), std::min(v[1].y(), v[2].y()));
    float ymax = std::max(v[0].y(), std::max(v[1].y(), v[2].y())) + 1;
    ...
    for (auto x = xmin + 0.5f; x <= xmax; x++) {
        for (auto y = ymin + 0.5f; y <= ymax; y++) {
