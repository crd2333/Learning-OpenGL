...
void Player::update(float time) {
    ...
    if (colorID != (int)teamid) diving = false;
    if (diving) {
    ...
    if (len > eps) {
        ...
        if (!jumpPhase) {
            ...
            else if (colorID == (int)teamid) A += - ourFloorA * dir;
...
int Player::getColorID(float x, float z) {
    ...
    for (int i = 0; i < (int)GameLogic::colors.size(); ++i) {
...
void GameLogic::init(int tNum, int pNum) {
    ...
    for (int i = 1; i < (int)Player::playerQueue.size(); ++i) {
