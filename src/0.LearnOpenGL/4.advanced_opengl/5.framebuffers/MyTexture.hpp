...
public:
    ...
    // image infomation
    int    width, height, nrChannels; // 图像的宽度、高度和通道数（由读取时 stb_image 返回）
    ...
    void GenerateEmpty(int width, int height) {
        glBindTexture(GL_TEXTURE_2D, ID);
        glTexImage2D(GL_TEXTURE_2D, 0, internal_format, width, height, 0, image_format, data_type, NULL);
        // set the texture filtering and wrapping parameters
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, filter_min);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, filter_max);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap_s);
...
public:
    ...
    void Generate(int width, int height) {
        glBindTexture(GL_TEXTURE_2D, ID);
        if (Multisample) {
            glTexImage2DMultisample(GL_TEXTURE_2D, 4, internal_format, width, height, GL_TRUE);
        } else {
